...
string aesencryptedjson(const string& plaintext);
// 接收线程
...
// 对称解密
string aesdecryptedjson (const string& plaintext);
...
int main(int argc, char **argv)
{
    ...
    json js;
    js["msgid"] = RSA_public;       
    string request = aesencryptedjson(js.dump());
    for (int i = 0;; i++)
    {
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            // 使用 RSA 公钥对密码进行加密
            unsigned char encrypted[rsa_len];
            int ret = RSA_public_encrypt(password.size(), 
                                  reinterpret_cast<const unsigned char*>(password.c_str()), 
                                  encrypted, pubkey, RSA_PKCS1_PADDING);
            
            // 将加密后的密码转换为字符串形式（Base64 编码）
            string encryptedPassword = base64_encode(encrypted, ret);

            js["password"] = encryptedPassword;
            
            //cerr << "生成之前的 JSON 请求字符串为：" << js.dump() << endl;
            string request = aesencryptedjson(js.dump());
            //cerr << "生成的 JSON 请求字符串为：" << request << endl;
...
{
    cout<<"整个离线消息"<<responsejs.dump()<<endl;
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        string temp = aesdecryptedjson(recv);
...
void addfriend(int clientfd, string str)
{
    ...
    string buffer = aesencryptedjson(js.dump());
...
void chat(int clientfd, string str)
{
    ...
    if (Friendlist.find(friendid)==Friendlist.end())
    {
        cerr << "not you friend" << endl;
        return;
    }
...
void groupchat(int clientfd, string str)
{
    ...
    if (Grouplist.find(groupid)==Friendlist.end())
    {
        cerr << "not in this group" << endl;
        return;
    }
...
string aesdecryptedjson(const string& ciphertext_hex){
    ...
    cout<<"收到服务器数据解密后的数据"<<plaintext_str<<endl;
